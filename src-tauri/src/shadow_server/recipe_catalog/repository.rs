...
pub fn list_recipes(conn: &Connection) -> Result<Vec<RecipeCatalogItem>, RepoError> {
    ...
    let ingredients_map = load_ingredients_batched(conn, &recipe_ids)?;
    let steps_map = load_steps_batched(conn, &recipe_ids)?;

    for recipe in &mut recipes {
        recipe.ingredients = ingredients_map.get(&recipe.id).cloned().unwrap_or_default();
        recipe.steps = steps_map.get(&recipe.id).cloned().unwrap_or_default();
    }
...
fn load_ingredients_batched(
    ...
    for row in rows {
        let (recipe_id, ingredient) = row.map_err(RepoError::from)?;
        map.entry(recipe_id)
...
fn load_steps_batched(
    ...
    let mut map = HashMap::new();
    for row in rows {
        let (recipe_id, step) = row.map_err(RepoError::from)?;
