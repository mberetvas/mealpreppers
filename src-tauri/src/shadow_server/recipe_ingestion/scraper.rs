...
use super::models::{RecipeDraft, RecipeIngredientDraft, RecipeSource, RecipeStepDraft, SUPPORTED_RECIPE_HOSTS};
use super::normalizers::{clean_text, parse_ingredient_line, parse_recipe_duration, parse_servings};
...
pub fn parse_json_ld_recipe(document: &Html, source: RecipeSource, recipe: Option<&Value>) -> RecipeDraft {
    let title = {
    ...
    let ingredient_strings = read_string_array(recipe.and_then(|r| {
        r.get("recipeIngredient").or_else(|| r.get("ingredients"))
    }));
    let ingredients: Vec<RecipeIngredientDraft> =
        ingredient_strings.iter().map(|s| parse_ingredient_line(s)).collect();

    let instruction_texts = extract_instruction_texts(recipe.and_then(|r| r.get("recipeInstructions")));
    let steps: Vec<RecipeStepDraft> = instruction_texts
        .into_iter()
        .map(|t| clean_text(&t))
        .filter(|t| !t.is_empty())
        .enumerate()
        .map(|(i, text)| RecipeStepDraft { position: i + 1, text })
        .collect();

    let categories = unique_strings(split_list(recipe.and_then(|r| r.get("recipeCategory")))
        .into_iter()
...
pub fn read_image_url(value: Option<&Value>) -> Option<String> {
    match value {
        ...
        Some(obj @ Value::Object(_)) => {
            obj["url"].as_str().map(|s| s.to_string())
        }
        _ => None,
    }
}
...
pub fn extract_instruction_texts(value: Option<&Value>) -> Vec<String> {
    match value {
        ...
        Some(obj @ Value::Object(map)) => {
            ...
            if map.contains_key("itemListElement") || map.contains_key("steps") || map.contains_key("step") {
                let nested = map.get("itemListElement")
