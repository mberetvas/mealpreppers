...
pub struct AppState {
    ...
    /// Recipe Catalog image filesystem port (wired in [`wire::wire_dependencies`]).
    pub recipe_images: Arc<dyn RecipeImageStore>,
    /// Consolidated shopping list persistence port (wired in [`wire::wire_dependencies`]).
