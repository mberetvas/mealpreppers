...
- [x] Drizzle schema/migrations cover planning and Saved Weekplan tables (additive to recipe schema from prior slice)
- [x] Saved Weekplan CRUD, month plan APIs, and planner mutations work E2E in desktop sidecar mode with SQLite file under app data (or `DATABASE_PATH` in dev)
- [x] Repository tests pass without Supabase mocks for planning modules touched in this slice
- [x] Domain terminology in code and tests uses **Saved Weekplan** (not “Week Template”)
